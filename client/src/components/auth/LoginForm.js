...
const LoginForm = ({ onClose, onSuccess, onSwitchToSignup, onForgotPassword }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          // Pass BOTH user + token to parent
    ...
    <>
      ...
      <Modal.Body className="login-modal-body pt-4">
        <Form onSubmit={formik.handleSubmit}>
          ...
          <Button variant="primary" type="submit" className="w-100 login-btn" disabled={isLoading}>
            {isLoading ? (
              <div className="spinner-border spinner-border-sm" role="status">
                <span className="visually-hidden">Loading...</span>
              </div>
